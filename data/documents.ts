...
const uploadToCloudinary = async (file: File): Promise<string> => {
    ...
    formData.append('folder', 'sanjose/documentos');
...
export const addDocument = async (
    ...
    try {
        ...
        logger.log("Generando miniatura...");
        ...
        logger.log("Subiendo archivo a Cloudinary...");
        ...
        // Solo agregar thumbnailBase64 si se generó correctamente
        if (thumbnailBase64) {
...
export const deleteDocumentItem = async (id: string, fileUrl: string): Promise<boolean> => {
    try {
        ...
        // Nota: El archivo en Cloudinary permanece (se puede limpiar manualmente desde
        // el dashboard de Cloudinary o implementando un endpoint serverless con el API Secret).
        logger.log("Documento eliminado de Firestore. Archivo en Cloudinary:", fileUrl);
        return true;
    } catch (error) {
...
// ─── Helpers de visualización (ahora usan la URL directa de Cloudinary) ───────

/**
 * Retorna directamente la fileUrl de Cloudinary almacenada en Firestore.
 * Ya no se necesita reconstruir el archivo desde chunks.
 */
export const getDocumentBlobUrl = async (docId: string): Promise<string | null> => {
    try {
        ...
        logger.error("Error obteniendo URL del documento", e);
        return null;
    }
};
