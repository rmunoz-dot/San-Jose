...
import { getDocuments, addDocument, deleteDocumentItem, DocumentItem, downloadDocumentFile, updateDocument } from '../data/documents';
import { Trash2, Plus, Upload, FileText, Calendar, Tag, ArrowLeft, LogOut, X, Eye, Edit2, Save, AlertTriangle, HelpCircle } from 'lucide-react';
...
const AdminUtp: React.FC = () => {
    const [documents, setDocuments] = useState<DocumentItem[]>([]);
    const [title, setTitle] = useState('');
    const [category, setCategory] = useState('Circulares (UTP)');
    ...
    const fetchDocuments = async () => {
        ...
        const utpDocs = fetchedDocs.filter(doc => doc.category?.includes('UTP'));
        setDocuments(utpDocs);
        setIsLoading(false);
    };
    ...
    const handleEditClick = (docItem: DocumentItem) => {
        ...
        // Desformatear la fecha chilena a YYYY-MM-DD
        // Esto es rudimentario, si es "5 de Enero, 2026", lo transformamos, pero si falla lo dejamos como fecha de hoy.
        const months = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
        let yyyy = new Date().getFullYear().toString();
        let mm = '01';
        let dd = '01';
        try {
            // ejemplo: 13 de Febrero, 2026
        ...
        <div className="min-h-screen bg-gray-50 py-24 relative">
            <div className="container mx-auto px-6 max-w-6xl">
                <div className="mb-10 text-center">
                    <h1 className="text-3xl font-bold font-heading text-school-gold">Panel de Administración UTP</h1>
                    <p className="text-gray-600 mt-2">Sube y organiza las planificaciones, circulares y material docente.</p>
                </div>

                <AdminTabBar setRunTour={setRunTour} handleLogout={handleLogout} />
                <AdminTour run={runTour} setRun={setRunTour} type="utp" />
                ...
                <div className="flex flex-col gap-12">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Ej: Proyecto Educativo Institucional (PEI)"
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        >
                                            ...
                                            <option value="Circulares (UTP)">Circulares (UTP)</option>
                                            <option value="Planificaciones (UTP)">Planificaciones (UTP)</option>
                                            <option value="Material Docente (UTP)">Material Docente (UTP)</option>
                                            <option value="Evaluaciones (UTP)">Evaluaciones (UTP)</option>
                                            <option value="Reglamentos Especiales (UTP)">Reglamentos Especiales (UTP)</option>
                                            <option value="Formularios (UTP)">Formularios (UTP)</option>
                                            <option value="Programación Anual (UTP)">Programación Anual (UTP)</option>
                                            <option value="Horarios de Curso (UTP)">Horarios de Curso (UTP)</option>
                                            <option value="Otros Documentos (UTP)">Otros Documentos (UTP)</option>
                                        </select>
                                    </div>
                                    <div>
                                ...
                                <div id="doc-form-file" className="mb-6">
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Archivo (PDF, Word, etc.)</label>
                                ...
                                >
                                    {isSubmitting ? 'Subiendo y analizando...' : (
                                        <>
                                            <Upload size={20} />
                                            Subir Documento
                                        </>
                    ...
                    <div id="doc-list" className="w-full space-y-4 border-t border-gray-200 pt-8">
                        <h2 className="text-xl font-bold font-heading text-gray-800 mb-6 flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-3 mb-3">
                                                ...
                                                {editingId === item.id ? (
                                                    <input
                                                        ...
                                                        className="font-bold text-gray-800 flex-1 border rounded px-2 py-1 text-sm bg-blue-50 focus:outline-none focus:ring-2 focus:ring-school-blue"
                                                    />
                                                ) : (
